...
def interactive_validation():
    ...
    try:
        ...
        print(f"\n🗃️ Reading data from MySQL table: {table}")
        ...
        print(f"🗃️ Target Rows: {result.total_rows_target:,}")
        print(f"🔬 Validations Run: {len(result.validation_results)}")
        print(f"⏱️ Total Execution Time: {sum(val.execution_time for val in result.validation_results):.3f}s")
        ...
        for val_result in result.validation_results:
            status_emoji = "✅" if val_result.status.value == "PASSED" else "❌" if val_result.status.value == "FAILED" else "⚠️"
            print(f"\n{status_emoji} {val_result.validation_type.upper()}: {val_result.status.value}")
            print(f"   📝 Message: {val_result.message}")
            print(f"   ⏱️ Execution time: {val_result.execution_time:.3f}s")
